...
public class MemberDietController {
   ...
   ///////////////식단 분석후 마이페이지에서 식단 확인
   @RequestMapping(value ="/Mypage/Nutrient",produces = "text/plain;charset=UTF-8")
   public String getNutrient(HttpServletRequest req,Model model) {
       String userID = (String) req.getSession().getAttribute("UserID");
       Map map = new HashMap();
       map.put("userID", userID);
       MemberDTO userinfo = service.selectOne(map);
