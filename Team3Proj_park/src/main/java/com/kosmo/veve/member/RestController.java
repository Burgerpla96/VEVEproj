...
public class RestController {
   ...
   //private String url = "http://172.30.1.14:8080/veve/upload/";
   ...
   public JSONObject androidlist(@RequestParam Map map) {
      ...
      for (int i = 0; i < gallaryFileDTO.size(); i++) {
         String file = url+gallaryFileDTO.get(i).getF_name();
         JSONObject row2 = new JSONObject();
         row2.put("gallary_no",gallaryFileDTO.get(i).getGallary_no());
         row2.put("userId", gallaryFileDTO.get(i).getUserID());
         row2.put("f_name", file);
         
         jArray2.add(i, row2);
      }

      jsonMain.put("sendData", jArray);
      jsonMain.put("sendData2", jArray2);
